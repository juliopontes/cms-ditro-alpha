...
Joomla.renderMessages = function(messages) {
	...
	jQuery.each(messages, function (type, item) {
		var div = jQuery('<div/>').addClass('alert').addClass('alert-' + type);
		var h4 = jQuery('<h4/>').addClass('alert-heading').text(Joomla.JText._(type));
		div.append(h4);

		var divList = jQuery('<div/>');
		jQuery.each(item, function (index, message) {
			var p = jQuery('<p/>').text(message);
			divList.append(p);
		});
		div.append(divList);
		jQuery('#system-message-container').append(div);
	});
